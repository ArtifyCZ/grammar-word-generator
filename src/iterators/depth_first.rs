...
impl<'grammar> Iterator for DepthFirstValidWordIterator<'grammar> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            let (rule_idx, replacement_idx) = self.current_rules.pop_back()?;
            assert!(rule_idx < self.rules.len());
            let replacements_len = self.rules[rule_idx].1.len();
            assert!(replacement_idx < replacements_len);
            if self.current_rules.len() >= self.recursion_limit {
                if let Some((rule_idx, replacement_idx)) = self.current_rules.pop_back() {
                    self.move_to_next(rule_idx, replacement_idx);
                }
                continue;
            }

            let current_string = self.current_rules.iter().fold(
            ...
            if current_string.symbols().all(|s| s.is_terminal()) {
                ...
                let path = self
                    .current_rules
                    .iter()
                    .map(|(rule_idx, _replacement_idx)| {
                        self.rules[*rule_idx].0.clone()
                    })
                    .collect();
                self.move_to_next(rule_idx, replacement_idx);
